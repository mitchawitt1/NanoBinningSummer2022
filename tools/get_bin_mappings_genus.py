...
def write_mappings_mm2(f2_file_path, mapping_file):
    mappings = []

    with open(f2_file_path) as f2_file:
        f2_file_reader = csv.reader(f2_file, delimiter="\t")
        f2_file_reader.__next__()
        for line in f2_file_reader:
            mappings.append([line[0], line[1]])

    with open(mapping_file, "w") as map_writer:
        map_writer.write("@Version:0.9.1\n")
        map_writer.write("@SampleID:_SAMPLEID_\n")
        map_writer.write("@@SEQUENCEID\tBINID\n")
        for mapping in mappings:
            map_writer.write(mapping[0] + "\t" + mapping[1] + "\n")
