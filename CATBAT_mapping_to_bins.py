...
"""
The purpose of this script is to take the mapping files generated from the CATBAT-based
binning method into actual fasta files that represent the subsequently generated bins.
The current structure of CATBAT (and MM2) files is:
...
# read in the CATBAT mappings file
CATBAT_mapping = pd.read_csv(map_file_loc, skiprows=3, header=None, sep="\t")

# specify the first entry, just so dictionary isn't empty
taxid_to_binnum = {CATBAT_mapping.iloc[0, 1]: 0}
num_bins = 1

# start populating a mapping for taxid to its bin number
for index, row in CATBAT_mapping.iterrows():
    tax_id = row.iloc[1]
...
for tax_id, binnum in taxid_to_binnum.items():
    ...
    headers = list(CATBAT_mapping.loc[CATBAT_mapping.iloc[:, 1] == tax_id].loc[:, 0])
